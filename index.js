...
const { createArrayTemplate } = require('ezito-babel-plugins/utils/function-name-creator'); 
...
function initialConfig(option = {}){
    if(!ezitoTypes.object(option)) option = {};
    ...
    customPluginsConfig.add(['ezito-babel-plugins/plugins/all', {
    ...
    customPluginsConfig.add(["ezito-babel-plugins/plugins/insert-function",{ prepareInsertFunction(){ 
        return [function hey( name ){ return {}} ]
    }, fileName : optionFileName}],);
    customPluginsConfig.add(["ezito-babel-plugins/plugins/auto-import" ,{
        prepareAutoImport(nodePath , fileName){
            fileName = fileName || optionFileName;
        },
        fileName : optionFileName
    }]);
    customPluginsConfig.add(["ezito-babel-plugins/plugins/add-source" , {
 ...
 */
function runDev( option ){ 
    if(!ezitoTypes.object(option)) option = {};
    if(process.env.NODE_ENV !== 'production' ){ 
        process.env.BABEL_DISABLE_CACHE = 0;
        const ctx = initialConfig(option.fileName)
